      ...
      <div style={cardStyle}>
        <div style={sectionStyle}>
          ...
          <div style={directionContainerStyle}>
            ...
            >
              <span style={directionIconStyle}>💱</span>
              原生币 → 事件代币
            </button>
            <button 
        ...
        <div style={sectionStyle}>
          ...
          {isLoading ? (
            ...
            <select style={selectStyle} value={selectedPoolId} onChange={e => setSelectedPoolId(e.target.value)}>
        ...
        {selectedPool && (
          <div style={poolInfoCardStyle}>
            ...
            <div style={poolDetailsStyle}>
              <div style={poolDetailItemStyle}>
                <span style={poolDetailLabelStyle}>费率</span>
                <span style={poolDetailValueStyle}>{(selectedPool.poolinit.fee / 10000).toFixed(2)}%</span>
              </div>
              <div style={poolDetailItemStyle}>
                <span style={poolDetailLabelStyle}>Tick 间距</span>
                <span style={poolDetailValueStyle}>{selectedPool.poolinit.tickSpacing}</span>
              </div>
              <div style={poolDetailItemStyle}>
                <span style={poolDetailLabelStyle}>当前 Tick</span>
                <span style={poolDetailValueStyle}>{selectedPool.poolinit.tick}</span>
              </div>
            </div>
          </div>
        )}
        ...
        {estimatedOutput && (
          <div style={outputCardStyle}>
            ...
            <div style={outputValueStyle}>
              {estimatedOutput} <span style={outputUnitStyle}>{outputUnit}</span>
            </div>
          </div>
        )}
        ...
        <div style={disclaimerStyle}>
          ⚠️ 该估价未考虑滑点与实际流动性影响，仅基于当前价格和费率计算
        </div>
      </div>
    </div>
  );
...
const directionContainerStyle: React.CSSProperties = {
  display: 'flex',
  gap: '12px',
  flexWrap: 'wrap',
};
...
const activeDirectionButtonStyle: React.CSSProperties = {
...
const directionIconStyle: React.CSSProperties = {
  fontSize: '18px',
};
...
const poolInfoCardStyle: React.CSSProperties = {
...
const poolInfoHeaderStyle: React.CSSProperties = {
  display: 'flex',
  alignItems: 'center',
  gap: '12px',
  marginBottom: '12px',
...
const poolDetailItemStyle: React.CSSProperties = {
  display: 'flex',
  flexDirection: 'column',
  gap: '4px',
};

const poolDetailLabelStyle: React.CSSProperties = {
  fontSize: '12px',
  color: '#6b7280',
};

const poolDetailValueStyle: React.CSSProperties = {
  fontSize: '14px',
  fontWeight: '600',
  color: '#374151',
};
...
const outputValueStyle: React.CSSProperties = {
  fontSize: '24px',
  fontWeight: '600',
  color: '#4f46e5',
  display: 'flex',
...
const outputUnitStyle: React.CSSProperties = {
  fontSize: '16px',
  fontWeight: '500',
  color: '#6366f1',
};
